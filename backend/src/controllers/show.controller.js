...
const createShow = async (req, res) => {
  try {
    ...
    if (!screen) {
      return res
        .status(404)
        .json({ message: "Screen not found in this theatre" });
    }
    ...
    if (overlappingShow) {
      return res.status(400).json({
        message: "Show time overlaps with an existing show on this screen.",
      });
    }
...
const updateShow = async (req, res) => {
  try {
    ...
    if (show.status === "cancelled") {
      return res
        .status(400)
        .json({ message: "Cannot update a cancelled show" });
    }

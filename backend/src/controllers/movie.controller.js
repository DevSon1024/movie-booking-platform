...
const createMovie = async (req, res) => {
  try {
    ...
    // Safety net: if a soft-deleted movie with the same title still exists in the DB
    // (from before hard-delete was in place), permanently remove it so the title is free.
    if (title) {
      const orphan = await Movie.findOne({
      ...
      if (orphan) {
        if (
          orphan.posterUrl &&
          orphan.posterUrl.includes("/api/movies/images/")
        ) {
          await deletePosterFile(path.basename(orphan.posterUrl));
        }
        await Movie.findByIdAndDelete(orphan._id);
      }
    }
    ...
    const movie = new Movie({
      ...
      streamingLinks:
        status === "ENDED" && streamingLinks ? parseJSON(streamingLinks) : [],
    });
  ...
  try {
    ...
    if (movie) {
      ...
      if (posterFile) {
        ...
        if (
          movie.posterUrl &&
          movie.posterUrl.includes("/api/movies/images/")
        ) {
...
const deleteMovie = async (req, res) => {
  ...
  if (movie) {
    // Delete the associated poster file from disk (if it is a locally stored file)
    if (movie.posterUrl && movie.posterUrl.includes("/api/movies/images/")) {
      await deletePosterFile(path.basename(movie.posterUrl));
    }

    // Permanently remove the document so the title is fully freed for reuse

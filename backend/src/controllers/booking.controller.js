...
import {
  isSeatLockedByAnotherUser,
  clearLocksAndEmitBooked,
...
const createBooking = async (req, res) => {
  ...
  const isSeatTaken = show.seats.some(
    (seat) =>
  ...
  if (isLockedByOthers) {
    res.status(400);
    throw new Error(
      "One or more selected seats are temporarily locked by another user.",
    );
  }
  ...
  try {
    ...
    const booking = new Booking({
      ...
      transactionId:
        transactionId || `TXN-DUMMY-${Math.floor(Math.random() * 1000000)}`,
    ...
    await createdBooking.populate(
      "movie",
      "title posterUrl duration language genre",
    );
    await createdBooking.populate({
      path: "show",
      populate: {
    ...
    try {
      ...
      freshShow.seats.forEach((seat) => {
        ...
        if (
          ...
          seat.userId?.toString() === req.user._id.toString()
        ) {
          seat.isBooked = false;
          seat.userId = null;
        }

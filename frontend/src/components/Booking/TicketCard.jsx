...
const TicketCard = ({
  ...
  return (
    <div
      className={`bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all border border-gray-100 dark:border-gray-700 flex ${viewMode === "grid" ? "flex-col" : "flex-col md:flex-row"}`}
    >
      <div
        className={`${viewMode === "grid" ? "w-full h-48 sm:h-64" : "md:w-48 h-48 md:h-auto"} relative shrink-0`}
      >
        <img
      ...
      <div className="flex-1 p-5 flex flex-col justify-between">
        <div>
          <div
            className={`flex justify-between items-start gap-2 ${viewMode === "grid" ? "flex-col" : ""}`}
          >
